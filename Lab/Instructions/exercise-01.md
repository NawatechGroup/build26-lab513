...
> [!Tip]
> Your SQL connection details are in `C:\creds.txt`, written by the provisioning script in Exercise 0. Open a terminal and run `Get-Content C:\creds.txt` to see the `SQL_SERVER`, `SQL_DB`, `SQL_ADMIN`, `SQL_PASSWORD`, and `LAB_INSTANCE_ID` values you need below.
